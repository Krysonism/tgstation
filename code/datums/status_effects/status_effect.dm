...
/datum/status_effect/stacking/on_apply()
	...
	status_overlay.transform = matrix() * (icon_height/WORLD_ICON_SIZE_VIRTUAL_HEIGHT) //scale the status's overlay size based on the target's icon size
	status_underlay.pixel_x = -owner.pixel_x
	status_underlay.transform = matrix() * (icon_height/WORLD_ICON_SIZE_VIRTUAL_HEIGHT) * 3
