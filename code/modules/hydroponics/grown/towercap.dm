	...
	if(grill)
		if(!user.combat_mode && !(W.item_flags & ABSTRACT))
			if(user.temporarilyRemoveItemFromInventory(W))
				...
				W.pixel_x = W.base_pixel_x + clamp(text2num(LAZYACCESS(modifiers, ICON_X)) - 16, -(WORLD_ICON_SIZE_WIDTH/2), WORLD_ICON_SIZE_WIDTH/2)
				W.pixel_y = W.base_pixel_y + clamp(text2num(LAZYACCESS(modifiers, ICON_Y)) - 16, -(WORLD_ICON_SIZE_VIRTUAL_HEIGHT/2), WORLD_ICON_SIZE_VIRTUAL_HEIGHT/2)
		else
			return ..()
